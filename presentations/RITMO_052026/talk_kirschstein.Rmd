...
- ökonomische Annahmen:
  ...
  - Ziel Spediteuer: Maximiere Kundenzufriedenheit
...
kable(tab1,
  ...
  row_spec(c(1, 7, 11, 15, 16, 19,20), bold = TRUE, background = "#f0f0f0")
```
]
...
plot_vrp_ggplot(tour.id = 1, aspect.ratio = 0.9)
```
]
--
.col-50[
...
plot_vrp_ggplot(tour.id = 6, aspect.ratio = 0.9)
```
]
...
plot_vrp_ggplot(tour.id = 3, aspect.ratio = 0.9)
```
]
--
.col-50[
...
- Formale Modellierung des präferenz-orientierten VRP mit gemischter Flotte
- Entwicklung exakter und heuristischer Lösungsverfahren
- Instanzgenerierung auf Basis realistischer Logistikdaten (Projekt: REWE, Einride)
- Simulationsrahmen für mehrperiodische Flottenübergangsszenarien
- Validierung und Benchmarking gegen Literaturansätze
